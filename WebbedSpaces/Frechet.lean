...
* Imported `WithSeminorms.summable_of_forall_le`: in a complete space whose topology is generated by a
  sequence of seminorms `p`, a series `∑ y k` is summable as soon as `p i (y k) ≤ g k` for
  `i ≤ k`, with `g` summable.
* `WithSeminorms.isStrictWeb_web`: the web of a sequence of seminorms generating the topology of
  a complete space is strict.
* `StrictlyWebbedSpace.of_completeSpace_firstCountable`: Fréchet spaces are strictly webbed.
...
/-- The **web of a sequence of seminorms**: the set attached to the finite sequence
`n₀, …, n_{k-1}` (stored with the newest index at the head) is
`{x | ∀ i < k, p i x ≤ n i + 1}`. -/
@[expose]
...
/-- **Fréchet spaces are strictly webbed**, Köthe II §35.1.(4). More generally, the result holds for every
complete, first-countable locally convex space, without a Hausdorff assumption. -/
instance StrictlyWebbedSpace.of_completeSpace_firstCountable [ContinuousSMul 𝕜 E]
